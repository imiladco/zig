...
final class Product_Section_Guard {
    ...
    private static function is_elementor_editing(): bool {
        /*
         * ادیتورِ المنتور، همون لحظه‌ای که باز می‌شود، یک آیفریمِ
         * «پیش‌نمایشِ زنده» لود می‌کند که مستقیماً به آدرسِ *فرانتِ* همین
         * صفحهٔ محصول می‌رود — یعنی نه ‎is_admin()‎ است، نه ‎wp_doing_ajax()‎.
         * آدرسِ آن آیفریم همیشه پارامترِ کوئریِ ‎elementor-preview‎ را دارد
         * (خودِ المنتور اضافه‌اش می‌کند، مستقل از هر شیءِ داخلی‌ای که هنوز
         * لود نشده). این چک را *قبل* از تکیه‌کردن به آبجکتِ ‎preview‎ی
         * المنتور می‌گذاریم چون آن یکی به زمان‌بندیِ initialize‌شدنِ
         * المنتور وابسته است — دقیقاً همان کلاس‌فرضی که رویِ این سایت دو
         * بار دیگر (‎get_doc_for_frontend‎، ‎documents/register_controls‎)
         * غلط از آب درآمد. اگر ‎template_redirect‎یِ ما زودتر از ست‌شدنِ
         * حالتِ پیش‌نمایشِ المنتور اجرا شود، تکیهٔ صرف به آبجکت باعث می‌شود
         * این نگهبان رویِ کلِ HTMLِ همان آیفریم (یعنی دقیقاً همان چیزی که
         * هر بار ادیتور باز می‌شود لود می‌شود) پارسِ سنگینِ DOMDocument
         * اجرا کند — هم کندیِ لودِ ادیتور، هم ریسکِ همان فاتالِ حافظه.
         */
