...
final class Product_Section_Settings {
    ...
    /**
     * محدود به سندِ Single Product تم‌بیلدر — نه هر صفحه/پستِ عادی، و نه
     * هر سندِ دیگری که تصادفاً «product» در نامِ کلاسش دارد (مثلِ سندِ
     * آرشیوِ محصول). قبلاً یک فال‌بکِ ‎stripos‎ رویِ نامِ کلاس هم بود که
     * این کنترل‌ها را رویِ سندهایِ نامرتبط هم ثبت می‌کرد — دقیقاً همان چیزی
     * که با ثبتِ کنترل‌های اضافه رویِ ‎TAB_SETTINGS‎ (همان تبی که «شروطِ
     * نمایش»یِ المنتورپرو هم آنجاست) باعث شد ذخیره‌سازیِ آن سندها با
     * خطایِ ۵۰۰ متوقف شود و شروطِ نمایش خراب/گم شود (v1.48.0). حالا فقط
     * تطبیقِ دقیقِ نامِ سند پذیرفته می‌شود.
     *
    ...
    private static function applies_to($document): bool {
        ...
        return 'product' === $document->get_name();
    }
