 ...
 * پس عمداً فقط یک لایه بازنویسی وجود دارد و تخت است: حذف یک گروه، افزودن
 * یک گروه، و «حتی اگر خالی بود نشان بده». هیچ لایهٔ دومی نیست و هیچ‌کدام
 * ساختار خودِ طرح را عوض نمی‌کنند.
 *
...
final class Filter_Schema {
     ...
     *
     * سه چیز می‌تواند عوض شود و بس: گروه حذف شود، گروهی اضافه شود، و گروهِ
     * خالی نشان داده شود یا نه. ترتیب، اپراتور و معنای گروه‌های خودِ طرح
     * دست نمی‌خورند — چون آن‌ها همان چیزی‌اند که «طرح مشترک» را مشترک نگه
     * می‌دارند. اگر دسته‌ای واقعاً ساختار متفاوتی می‌خواهد، طرح خودش را
     * دارد؛ نه یک زنجیرهٔ بازنویسی که کسی نتواند دنبالش را بگیرد.
     *
     * افزودن، نشانِ صریح می‌خواهد (‎added‎). بدون آن نمی‌شد بین «مدیر این
     * را عمداً اضافه کرده» و «این ارجاع از یک گروهِ پاک‌شده مانده» فرق
     * گذاشت — و آن‌وقت یا هشدارِ دروغ می‌دادیم یا یک تنظیمِ بی‌اثر را بی‌صدا
     * می‌بلعیدیم.
     *
    ...
    private static function apply_overrides(array $facets, array $overrides): array {
        ...
        foreach ($overrides as $taxonomy => $rule) {
            ...
            if (!empty($rule['added'])) {
                ++$applied;
                $added[] = ['taxonomy' => $taxonomy, 'show_empty' => !empty($rule['show_empty'])];
        ...
        // افزوده‌ها ته فهرست می‌نشینند، تا ترتیبی که طرح ساخته به‌هم نریزد
        return [array_merge($out, self::sanitize_facets($added)), $applied, $stale];
    }
