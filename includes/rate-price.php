...
final class Rate_Price {
    ...
    /**
     * همان زمان، آماده برایِ نمایش — تاریخ با فرمتِ خودِ سایت، ساعت همیشه ۲۴ساعته.
     *
     * تاریخ همان الگویی را دنبال می‌کند که بقیهٔ افزونه به کار می‌برد
     * (‎software-info-table.php‎، ‎documents.php‎): ‎wp_date()‎ با فرمتِ
     * تنظیماتِ وردپرس، نه یک فرمتِ ثابت اینجا — اگر سایت افزونهٔ تقویمِ
     * جلالی دارد که رویِ ‎wp_date‎/‎date_i18n‎ فیلتر می‌گذارد، همان تبدیل
     * خودش اینجا هم اثر می‌کند؛ این کلاس محاسبهٔ تقویم را دوباره نمی‌سازد.
     *
     * ساعت برخلافِ تاریخ به تنظیماتِ سایت وابسته نیست: طرح صریحاً ۲۴ساعته
     * می‌خواهد (بدونِ ‎ب.ظ‎/‎ق.ظ‎)، پس ‎H:i‎ همیشه همین‌جا هارد‌کد است، نه
     * ‎get_option('time_format')‎ — که پیش‌فرضِ خودِ وردپرس ۱۲ساعته است و
     * قبلاً همان چیزی چاپ می‌شد.
     */
