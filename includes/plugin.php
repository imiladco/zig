...
final class Plugin {
    ...
    public function boot_filters(): void {
        ...
        if (!is_admin()) {
            ...
            /*
             * ‎Product_Section_Guard::boot()‎ عمداً موقتاً صدا زده نمی‌شود.
             * کاربر رویِ سایت با «Commands out of sync» در لاگِ MySQL
             * (رویِ هوکِ shutdown، از کوئری‌هایِ کاملاً بی‌ربطِ ووکامرس/
             * ActionScheduler) و ادیتورِ المنتور که رویِ صفحهٔ محصول اصلاً
             * لود نمی‌شود مواجه شده — حتی بعدِ دو دور فیکسِ حافظه/تشخیصِ
             * پیش‌نمایش (v1.49.0، v1.50.0). این خطا نشانهٔ یک کانکشنِ
             * MySQLِ خراب‌شده (مثلاً از یک workerِ کشته‌شدهٔ وسطِ کوئری)
             * است، نه یک باگِ منطقی، و چون فقط رویِ صفحهٔ محصول رخ می‌دهد
             * (جایی که این کلاس تنها کدِ اضافه‌شدهٔ اخیر است)، برایِ
             * ایزوله‌کردنِ قطعیِ علت باید کاملاً کنار گذاشته شود — نه فقط
             * محافظتِ بیشتر. فایل رویِ دیسک می‌ماند (کارِ خرابی نیست)
             * ولی هوکش قطع است، دقیقاً همان الگویی که برایِ
             * ‎Product_Section_Settings‎ هم به‌کار رفت.
             */
        }
    }
