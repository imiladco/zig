...
final class Plugin {
    ...
    public function boot_filters(): void {
        ...
        if (!is_admin()) {
            ...
            /*
             * ‎Product_Section_Guard::boot()‎ در v1.51.0 موقتاً کنار
             * گذاشته شده بود تا فاتالِ حافظه (OOM رویِ ‎admin-ajax.php‎)
             * ایزوله شود. لاگِ واقعیِ بعدی (با اکشنِ ‎ajax‎ و آخرین کوئریِ
             * ‎wpdb‎) نشان داد آن فاتال دقیقاً همان لحظه‌ای رخ می‌دهد که
             * اکشنِ ‎elementor_ajax‎ (یعنی خودِ ذخیره‌سازیِ سند در ادیتور)
             * در حالِ اجراست — یک مسیرِ کاملاً مجزا از این کلاس، که فقط
             * رویِ ‎template_redirect‎یِ بازدیدِ *سایت* کار می‌کند و به
             * صراحت از حالتِ ادیتور/پیش‌نمایش کنار می‌کشد (نگاه کنید به
             * ‎is_elementor_editing()‎). این کلاس هیچ نقشی در پردازشِ
             * ‎elementor_ajax‎ ندارد، پس دوباره فعال شد.
             */
