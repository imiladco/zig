...
final class Plugin {
    ...
    public function boot_filters(): void {
        ...
        if (!is_admin()) {
            ...
            /*
             * ‎Product_Section_Guard::boot()‎ دوباره در v1.53.0 کنار
             * گذاشته شد — این‌بار به دلیلِ کاملاً متفاوت و جدی‌تر: بعدِ
             * فعال‌سازیِ دوباره‌اش در v1.52.0، کاربر گزارش کرد متنِ صفحاتِ
             * محصول (فارسی) به‌شکلِ عجیب/خرابی درآمده — دقیقاً همان
             * ریسکی که در کامنتِ خودِ ‎filter_html()‎ ثبت شده بود:
             * ‎mb_convert_encoding($out, 'UTF-8', 'HTML-ENTITIES')‎ که
             * برایِ برگردوندنِ موجودیت‌هایِ عددیِ ‎saveHTML()‎ به UTF-8
             * استفاده می‌شود، رویِ HTMLِ واقعیِ کاملِ صفحه (نه قطعه‌هایِ
             * سینتتیکِ تست) با محتوایِ فارسی/اسکریپت/استایلِ واقعی رفتارِ
             * قابلِ‌اعتمادی نشان نداد. چون این افزونه اساساً یک سایتِ
             * تمام‌فارسی است، این کلاس تا اصلاحِ ریشه‌ایِ همین تبدیلِ
             * انکودینگ (یا جایگزینیِ کاملِ روشِ خروجی‌گیری) نباید فعال
             * باشد — بدونِ استثنا.
             */
        }
    }
