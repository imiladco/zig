...
final class Search_Normalizer {
    ...
    public static function numeric_variants(string $normalized): array {
        ...
        /*
         * شکلِ فاصله‌دارِ *همان* خط — چیزی که Tier 2 نمی‌توانست بسازد.
         *
         * ‎joining_variants()‎ عمداً رویِ «سهبعدی» ساکت است، چون بدونِ
         * فاصله یا ZWNJ هیچ مرزِ کلمه‌ای معلوم نیست. ولی این‌جا مرز
         * *معلوم* است: یک عددِ شناخته‌شده که به یک پسوندِ شناخته‌شده چسبیده.
         * پس همان جدایی‌ای که آن‌جا ممکن نبود، این‌جا با قطعیت ساخته می‌شود.
         *
         * بدونِ این دو قاعده، «سهبعدی» فقط «3 بعدی» می‌ساخت و «3بعدی» فقط
         * «سه بعدی» — یعنی کاربری که چسبیده تایپ می‌کند، محصولی را که
         * ادمین با فاصله و در همان خط نوشته پیدا نمی‌کرد.
         */
