...
final class Facets {
     ...
     * ابزارک قدیمی ناوبری لایه‌ای ووکامرس با جدول جست‌وجو حالت دوم را هم
     * پیاده کرده، ولی مسیر امروزی خودِ ووکامرس هم مثل ما در سطح محصول کار
     * می‌کند. تفاوتشان فقط روی محصول متغیری دیده می‌شود که دو ترم را روی دو
     * گزینهٔ متفاوت دارد.
     */
     ...
     *
     * چون خودِ ووکامرس هم نمی‌کند. مسیر فعلیِ شمارش ویژگی‌ها در ووکامرس
     ...
     *
     * جدول جست‌وجو هنوز در ‎Filterer‎ استفاده می‌شود، ولی آن مسیرِ ابزارکِ
     * قدیمیِ ناوبری لایه‌ای است، نه مسیر فیلترهای امروزی.
     *
     * داشتن دو مسیر که باید عدد یکسان بدهند، بدون تست یکپارچگی روی محصول
     * متغیر، ریسک است بدون سود اثبات‌شده.
     *
     * ---
     *
     * ‎in_stock‎ هم عمداً نیست.
     *
     * ‎Filterer‎ (مسیر قدیمی) وقتی «پنهان‌کردن ناموجودها» روشن باشد
     * ‎AND in_stock = 1‎ می‌گذارد و با آن، محصولی که *گزینهٔ متناظر با این
     * ترم*‌اش ناموجود است کنار می‌رود. مسیر امروزیِ ووکامرس این کار را
     * نمی‌کند و ما هم نمی‌کنیم.
     *
     * دلیلش یک قاعدهٔ سفت است: شمارش باید دقیقاً همان قیدهایی را داشته باشد
     * که فهرستِ کنارش دارد. مجموعهٔ پایهٔ ما از همان ‎WP_Query‎ می‌آید که خودِ
     * فهرست را می‌سازد و در سطح محصول فیلتر می‌کند، نه در سطح گزینه. اگر
     * این قید فقط به شمارش اضافه شود، کاربر «۷» را می‌بیند، کلیک می‌کند و
     * ۹ محصول می‌آید.
     *
     * این یک انتخاب معنایی است، نه سهو: معنای فیلتر ما «این محصول این ترم
     * را دارد» است، نه «گزینهٔ موجودی با این ترم دارد». اگر روزی دومی
     * خواسته شد، باید هم‌زمان به فهرست و شمارش اعمال شود — نه فقط یکی.
     *
