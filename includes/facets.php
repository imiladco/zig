...
final class Facets {
     ...
     *
     * ‎$in_stock_only‎ به‌صورت پیش‌فرض خاموش است و باید بماند.
     *
     * وسوسه‌اش هست — ووکامرس خودش وقتی «پنهان‌کردن ناموجودها» روشن باشد
     * ‎AND in_stock = 1‎ می‌گذارد. ولی ما آن را اینجا تکرار نمی‌کنیم، چون
     * مجموعهٔ پایهٔ ما از همان ‎WP_Query‎ می‌آید که خودِ فهرست را می‌سازد و
     * آن کوئری در سطح محصول فیلتر می‌کند، نه در سطح گزینه. اگر این قید را
     * فقط به شمارش اضافه کنیم، عددها از فهرستی که کنارشان رندر شده کمتر
     * می‌شوند: کاربر «۷» را می‌بیند، کلیک می‌کند و ۹ محصول می‌آید.
     *
     * ناسازگاریِ شمارش با فهرستی که کنارش نشسته، بدتر از هر بی‌دقتیِ
     * نظری است. پس قاعده این است: شمارش دقیقاً همان قیدهایی را دارد که
     * فهرست دارد — نه یکی کمتر، نه یکی بیشتر.
     *
     * ترتیب شرط‌ها اتفاقی نیست. ووکامرس روی این جدول یک ایندکس ترکیبی دارد:
     *
     *     (taxonomy, term_id, in_stock, product_or_parent_id)
     *
     * کوئری ما با ‎taxonomy‎ شروع می‌شود (ستون پیشروی ایندکس)، روی
     * ‎term_id‎ گروه می‌بندد و به ‎product_or_parent_id‎ نگاه می‌کند — یعنی
     * هر چهار ستون داخل همان ایندکس‌اند و اصلاً لازم نیست دیتابیس سراغ
     * ردیف‌های جدول برود.
     *
     ...
     *
     * برخلاف تصور رایج، این مسیر برای کاری که ما می‌کنیم *کم‌دقت‌تر نیست*.
     * دلیلش شکل کوئری ماست: مجموعهٔ پایه از یک ‎WP_Query‎ می‌آید که در سطح
     * محصولِ والد فیلتر می‌کند، و هر دو مسیر دقیقاً یک چیز را می‌شمارند —
     * «چند محصولِ والدِ داخل مجموعهٔ پایه این ترم را دارند». جدول جست‌وجو
     * برای محصول متغیر هم ‎product_or_parent_id‎ را همان والد می‌گذارد، پس
     * ‎COUNT(DISTINCT …)‎ در هر دو به یک عدد می‌رسد.
     *
     * تفاوت واقعی فقط یک جاست: ستون ‎in_stock‎، که اجازه می‌دهد محصولی که
     * *گزینهٔ متناظر با این ترم*‌اش ناموجود است کنار گذاشته شود. آن قید
     * اینجا اصلاً قابل بیان نیست — و ما هم به‌صورت پیش‌فرض از آن استفاده
     * نمی‌کنیم (به ‎count_sql()‎ نگاه کنید).
     *
     * پس مسیر جایگزین کندتر است، نه غلط‌تر.
     */
