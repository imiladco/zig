...
/* ==========================================================================
 * for_term() — ‎0‎ یعنی «کلِ فروشگاه»، نه شناسهٔ نامعتبر
 * ======================================================================= */

Tests::group('ذخیره‌سازی › for_term');

/*
 * قبلاً هر شناسهٔ کوچک‌تر از ‎۱‎ (یعنی هم منفی و هم ‎۰‎) همین‌جا [] می‌گرفت.
 * ‎۰‎ حالا معنایِ مستقلی دارد — «هیچ دسته‌ای، کلِ کاتالوگ» — و باید از
 * همان مسیرِ AUTO/discover رد شود، نه این‌جا کوتاه شود؛ فقط شناسهٔ واقعاً
 * نامعتبر (منفی) هنوز پیش از هر کاری [] می‌دهد.
 */
Tests::same('شناسهٔ منفی، آرایهٔ خالی — پیش از هر کوئری‌ای', Schema_Store::for_term(-1), []);

/*
 * ‎require_once‎های این فایل به Archive_Query/Attributes هم نیاز دارند تا
 * for_term(0) واقعاً اجرا شود؛ در این سوییتِ کوچک هیچ ویژگیِ ووکامرسی ثبت
 * نشده (‎wc_get_attribute_taxonomy_names()‎ استاب‌نشده)، پس کشف چیزی پیدا
 * نمی‌کند و پیش از رسیدن به ‎$wpdb‎ (که اینجا استاب هم نیست) با [] برمی‌گردد
 * — یعنی این سنجه فقط ثابت می‌کند for_term(0) دیگر زودتر رد نمی‌شود و
 * فاتال هم نمی‌گیرد؛ خودِ رفتارِ «بدونِ قیدِ دسته» را
 * ‎archive-query-test.php‎ رویِ ‎base_args()‎ مستقیم می‌سنجد.
 */
require_once __DIR__ . '/lib/woocommerce-stub.php';
require_once dirname(__DIR__) . '/includes/attributes.php';
require_once dirname(__DIR__) . '/includes/archive-query.php';

Tests::same('شناسهٔ صفر، بدونِ فاتال از مسیرِ AUTO رد می‌شود', Schema_Store::for_term(0), []);
