...
/* --------------------------------------------------------------------------
 * جداییِ پایه از ترتیب و صفحه
 *
 * این فقط تمیزکاری نیست، شرط درستیِ شمارش است. اگر paged وارد کوئری پایه
 * می‌شد، زیرکوئریِ شمارش فقط محصولات همان صفحه را می‌شمرد — عددی که همیشه
 * از posts_per_page کوچک‌تر است و کاملاً هم معقول به نظر می‌رسد. و ترتیب،
 * برای شمارش بی‌اثر است ولی JOIN سنگین می‌آورد.
 * ----------------------------------------------------------------------- */

$clean = Archive_Query::base_args(['categories' => [12]]);

Tests::ok('کوئری پایه صفحه ندارد', !isset($clean['paged']));
Tests::ok('و تعداد در صفحه ندارد', !isset($clean['posts_per_page']));
Tests::ok('و ترتیب ندارد', !isset($clean['orderby']) && !isset($clean['meta_key']));
