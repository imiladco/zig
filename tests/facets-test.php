...
/* ==========================================================================
 * نمایش گزینه‌های بدون نتیجه
 * ======================================================================= */

Tests::group('فست › گزینه‌های صفر');

$mixed = Facets::options($terms, ['up3d' => 4, 'vhf' => 0, 'aidite' => 0], []);

Tests::same('پیش‌فرض: همه می‌مانند', count(Facets::visible_options($mixed)), count($mixed));

Tests::same(
    'خاموش که باشد، فقط گزینهٔ نتیجه‌دار می‌ماند',
    array_column(Facets::visible_options($mixed, false), 'slug'),
    ['up3d']
);

/*
 * قیدی که کاربر ببیندش ولی نتواند برش دارد، بن‌بست است. گزینهٔ فعالِ صفر
 * باید بماند حتی وقتی «گزینه‌های بدون نتیجه» خاموش است.
 */
$selected_zero = Facets::options($terms, ['up3d' => 0, 'vhf' => 0, 'aidite' => 0], ['vhf']);

Tests::same(
    'گزینهٔ انتخاب‌شده هیچ‌وقت حذف نمی‌شود',
    array_column(Facets::visible_options($selected_zero, false), 'slug'),
    ['vhf']
);

/*
 * نبودِ شمارش (کش سرد، شمارش خاموش) یعنی «نمی‌دانم»، نه «صفر». حذف بر
 * پایهٔ ندانستن، فهرست را بی‌دلیل خالی می‌کرد.
 */
Tests::same(
    'وقتی شمارشی در کار نیست، چیزی حذف نمی‌شود',
    count(Facets::visible_options(Facets::options($terms, null, []), false)),
    count($terms)
);
