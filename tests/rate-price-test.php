...
/*
 * ‎date_format‎ِ تنظیماتِ سایت عمداً به چیزی غیرِ ‎j F Y‎ ست می‌شود تا اگر
 * روزی کسی دوباره ‎get_option('date_format')‎ را برایِ تاریخ اضافه کند،
 * این سنجه قرمز شود — تاریخ باید همیشه روز-ماه-سال باشد، نه فرمتِ سایت.
 */
$GLOBALS['__zig_options']['date_format'] = 'Y-m-d';
$GLOBALS['__zig_options']['time_format'] = 'H:i';

Tests::keeps('تاریخ همیشه روز-ماه-سال است، نه فرمتِ سایت', Rate_Price::display(501), date('j F Y', 1_700_000_000));
 ...
 */
Tests::keeps('همان فرمتِ ثابت، بدونِ خواندنِ دوباره', Rate_Price::format(1_700_000_000), date('j F Y', 1_700_000_000));
...
Tests::same(
    ...
    date('j F Y', 1_700_000_000) . ' , ' . date('H:i', 1_700_000_000)
);

/* ==========================================================================
 * fallback به تاریخِ ذخیرهٔ خودِ محصول
 * ======================================================================= */

/*
 * ووکامرس هیچ تاریخچهٔ واقعیِ تغییرِ قیمت ندارد؛ برایِ محصولی که نرخ‌محور
 * نیست، تنها گزینهٔ بعدی ‎post_modified‎ِ خودِ محصول است — همان چیزی که
 * ‎get_date_modified()‎ برمی‌گرداند.
 */

Tests::group('زمانِ به‌روزرسانیِ قیمت › fallback به تاریخِ ذخیرهٔ محصول');

$GLOBALS['__zig_mns_products'][801] = ['active' => 'yes', 'currency_id' => 7];

$rate_based = new WC_Product(['id' => 801, 'modified' => 1_650_000_000]);

Tests::same(
    'وقتی نوسان چیزی دارد، همان برنده است، نه تاریخِ ذخیره',
    Rate_Price::updated_at_for($rate_based),
    1_700_000_000
);

/* محصولِ ۸۰۲ در نوسان اصلاً ثبت نشده — یعنی updated_at() صفر می‌دهد */
$not_rate_based = new WC_Product(['id' => 802, 'modified' => 1_650_000_000]);

Tests::same(
    'بدونِ نوسان، تاریخِ ذخیرهٔ محصول جایگزین می‌شود',
    Rate_Price::updated_at_for($not_rate_based),
    1_650_000_000
);

/* نه نوسان، نه حتی تاریخِ ذخیره — محصولی که هیچ‌وقت ذخیره نشده (فرضی) */
$never_saved = new WC_Product(['id' => 803]);

Tests::same(
    'بدونِ هیچ‌کدام هم صفر می‌دهد، نه خطا',
    Rate_Price::updated_at_for($never_saved),
    0
);
