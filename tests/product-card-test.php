...
/*
 * «فهرست خالی» یعنی حالت خودکار — و ‎['']‎ هم خالی است.
 *
 * این باگ روی نصب واقعی پیدا شد و از تستِ رندر رد می‌شد: کنترل
 * چندانتخابیِ المنتور وقتی چیزی انتخاب نشده ‎''‎ می‌دهد نه ‎[]‎، و
 * ‎(array) ''‎ می‌شود ‎['']‎ — آرایه‌ای که شرطِ «خالی؟» را رد می‌کند. نتیجه
 * این بود که نوار ویژگی‌ها روی *هر* سایتی که این فیلد را دست نزده بود
 * بی‌صدا غیبش می‌زد؛ کارت هم بدون آن، سالم به نظر می‌رسید.
 */
foreach ([[''], ['', '  '], [null]] as $index => $blank) {
    Tests::same(
        'فهرستِ عملاً خالی ⇒ همان حالت خودکار (' . $index . ')',
        Product_Card::data(WC_Product::$registry[60], ['features_attrs' => $blank])['features'],
        ['خشک']
    );
}
